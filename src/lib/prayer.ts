...
export interface PrayerSlot {
  name: PrayerName;
  azan: string | null; // "HH:MM" or "HH:MM:SS"
  jamaat: string | null;
}

export interface PrayerRow {
  date: string;
  fajr_azan: string | null; fajr_jamaat: string | null;
  sunrise: string | null;
  zuhr_azan: string | null; zuhr_jamaat: string | null;
  asr_azan: string | null;  asr_jamaat: string | null;
  maghrib_azan: string | null; maghrib_jamaat: string | null;
  isha_azan: string | null; isha_jamaat: string | null;
  jumuah_1: string | null; jumuah_2: string | null; jumuah_3: string | null;
}
...
export function currentAndNextPrayer(row: PrayerRow | null | undefined, now = new Date()) {
  if (!row) return { current: null as PrayerSlot | null, next: null as PrayerSlot | null, msToNext: 0 };
  ...
  for (let i = 0; i < slots.length; i++) {
    ...
    else { next = slots[i]; break; }
  }
  if (!next) next = slots[0]; // tomorrow's Fajr
...
// Tabular Hijri approximation (Umm al-Qura-ish). Not for fiqh decisions.
