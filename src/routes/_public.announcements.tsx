...
export const Route = createFileRoute("/_public/announcements")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Latest news and announcements from the masjid administration." },
    ],
  }),
      ...
      <div className="mt-10 space-y-5">
        {(data ?? []).map((a) => (
          <article key={a.id} className="rounded-2xl border border-border bg-card p-7 shadow-sm">
            <div className="text-xs uppercase tracking-wider text-muted-foreground">{new Date(a.created_at).toLocaleDateString()}</div>
        ...
        {data?.length === 0 && <p className="text-muted-foreground">No announcements at this time.</p>}
      </div>
    </div>
  );
}
