...
export const Route = createFileRoute("/_public/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Daily Azan, Jamaat times, monthly prayer timetable, Jumuah and community events at Al-Noor Masjid." },
    ],
  }),
  component: HomePage,
});
    ...
    <>
        ...
        <div className="container relative mx-auto grid gap-12 px-4 py-20 md:grid-cols-2 md:py-28">
          <div>
            ...
            <p className="mt-6 max-w-lg text-lg text-primary-foreground/75">
              Daily Azan and Jamaat times, monthly schedule and community announcements — kept current by the masjid administration.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Button asChild size="lg" className="bg-gradient-gold text-gold-foreground shadow-gold hover:opacity-95">
                <Link to="/prayer-times">View today's times <ArrowRight className="ml-2 h-4 w-4" /></Link>
              </Button>
              <Button asChild size="lg" variant="outline" className="border-gold/40 bg-transparent text-primary-foreground hover:bg-gold/10">
          ...
          <div className="relative">
            <div className="rounded-3xl border border-gold/20 bg-card/95 p-8 text-card-foreground shadow-elegant backdrop-blur">
              <div className="flex items-center justify-between">
                <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Next Prayer</div>
                <div className="text-xs text-muted-foreground">{now.toLocaleString(undefined,{weekday:"long",month:"long",day:"numeric"})}</div>
              </div>
              <div className="mt-3 flex items-baseline justify-between">
                <div className="font-display text-6xl text-primary">{next?.name ?? "—"}</div>
                <div className="font-display text-3xl text-gold">{formatTime12(next?.azan ?? null)}</div>
              </div>
              <div className="mt-6 rounded-2xl bg-muted/60 p-6 text-center">
                <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Time remaining</div>
                <div className="mt-2 font-display text-5xl tabular-nums text-primary">{formatCountdown(msToNext)}</div>
              </div>
              {current && (
                <div className="mt-4 flex items-center justify-between rounded-xl border border-border/60 px-4 py-3 text-sm">
                  <span className="text-muted-foreground">Now</span>
                  <span className="font-medium">{current.name} · {formatTime12(current.azan)}</span>
                </div>
              )}
            </div>
          </div>
      ...
      <section className="container mx-auto px-4 py-20">
        <div className="flex items-end justify-between gap-4">
          <div>
            <div className="text-xs uppercase tracking-[0.2em] text-gold">Today</div>
            <h2 className="mt-2 font-display text-4xl md:text-5xl">Today's prayer times</h2>
          </div>
          <Link to="/monthly-timetable" className="hidden text-sm font-medium text-primary hover:underline md:inline">View full month →</Link>
        </div>
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          {slots.map((s) => (
            <div key={s.name} className="group rounded-2xl border border-border bg-card p-5 transition-all hover:-translate-y-1 hover:shadow-elegant">
              <div className="flex items-center justify-between">
                <div className="font-display text-xl">{s.name}</div>
                <Clock className="h-4 w-4 text-gold" />
              </div>
              <div className="mt-3 font-display text-3xl text-primary">{formatTime12(s.azan)}</div>
              {s.jamaat && <div className="mt-1 text-xs text-muted-foreground">Jamaat · {formatTime12(s.jamaat)}</div>}
            </div>
          ))}
          {slots.length === 0 && <p className="text-muted-foreground">No timetable set for today. Please check back soon.</p>}
        </div>
      </section>
        ...
        <div className="container mx-auto grid gap-12 px-4 lg:grid-cols-2">
          <div>
            ...
            <div className="mt-8 space-y-4">
              ...
              {announcements?.length === 0 && <p className="text-muted-foreground">No announcements at this time.</p>}
            </div>
          </div>
          <div>
              ...
              {(events ?? []).map((e) => (
                <div key={e.id} className="rounded-2xl border border-border bg-card p-6 shadow-sm">
                  ...
                  <div className="mt-4 flex flex-wrap gap-4 text-xs text-muted-foreground">
                    {e.start_time && <span className="inline-flex items-center gap-1"><Clock className="h-3 w-3" /> {formatTime12(e.start_time)}{e.end_time && ` – ${formatTime12(e.end_time)}`}</span>}
                    {e.location && <span className="inline-flex items-center gap-1"><MapPin className="h-3 w-3" /> {e.location}</span>}
                    <span className="inline-flex items-center gap-1"><Calendar className="h-3 w-3" /> {new Date(e.event_date).toLocaleDateString(undefined,{weekday:"long"})}</span>
                  </div>
                </div>
              ))}
