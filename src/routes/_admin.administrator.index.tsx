...
import { Bell, Calendar, ClockIcon, Images, Users } from "lucide-react";
...
function Dashboard() {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const [pt, an, ev, gallery] = await Promise.all([
        supabase.from("prayer_times").select("id", {
          count: "exact",
          head: true,
        }),
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Stat icon={Images} label="Gallery images" value={data?.gallery ?? "—"} />
      </div>
    </div>
  );
}
